...
// The origin comes from libs/site.ts, which guards a blank or unparseable env
// var and strips the trailing slash — this module used to carry its own
// expression, and that is exactly the drift site.ts exists to end.
...
export const resolveLocale = (value: unknown): Locale =>
	LOCALES.find((locale) => locale === value) ?? DEFAULT_LOCALE;
...
export const withLocale = (locale: string | undefined, path: string): string =>
	locale === DEFAULT_LOCALE || locale === undefined
		? path
		: `/${locale}${path === "/" ? "" : path}`;

// The same, absolute.
export const pageUrl = (locale: string | undefined, path: string): string =>
	`${HOST}${withLocale(locale, path)}`;
...
export const markdownSibling = (publicPath: string | null | undefined): string => {
...
export const markdownUrl = (locale: string | undefined, path: string): string =>
...
export const routePath = (value: unknown): string => {
