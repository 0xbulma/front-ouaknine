import { isMarkdownPath, isNegotiablePath, preferredType } from './accept.mjs';
import { markdownUrl, withLocale } from './site-url.mjs';

// What the middleware should do with a request, as a value.
//
// The decision lives here rather than in middleware.js because `node --test`
// cannot import that file, and the branch *ordering* is what has repeatedly
// broken: a redirect built from the locale-stripped path, a data guard placed
// above the branches it then preempted, a variant served without naming its
// discriminator in `Vary`. None of those are visible in a leaf primitive; all
// of them are one table-driven case here.
//
// middleware.js turns the tag into a NextResponse and does nothing else.
...
const rewrite = (route, path) => ({ kind: 'rewrite', route, path });

export const route = ({ pathname, search = '', locale = 'fr', accept, isData = false }) => {
  ...
  if (WELL_KNOWN[pathname]) {
    return search ? redirect() : rewrite(WELL_KNOWN[pathname]);
  }
