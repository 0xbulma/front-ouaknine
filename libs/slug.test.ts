...
import { splitTitle, seriesOf, isPress } from './publication-fields';
...
test('slugify strips accents', () => {
  ...
  ).toBe('guide-de-survie-en-garde-a-vue-episode-1-connaitre-ses-droits');
});
...
test('internalPath keeps a same-site link on the site', () => {
  expect(internalPath('https://www.ouaknine-avocats.com/articles/abc')).toBe(
    '/articles/abc'
  );
  ...
  expect(internalPath('https://ouaknine-avocats.com/publications/x')).toBe(
    '/publications/x'
  );
});
...
test('a bad NEXT_PUBLIC_HOST falls back instead of poisoning every URL', async () => {
  for (const bad of ['', 'ouaknine-avocats.com', 'mailto:a@b.c', 'javascript:1']) {
    ...
    expect(probe.SITE_HOSTS).toStrictEqual([
      'ouaknine-avocats.com',
      'www.ouaknine-avocats.com',
    ]);
...
test('SITE_HOSTS is the same pair whichever spelling the env var uses', async () => {
  const read = (host: string) =>
    withHost(host, async () => (await import('./site')).SITE_HOSTS);
