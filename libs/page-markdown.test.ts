...
test('the expertise index links every field at its own URL', () => {
  ...
  expect(
    markdown.includes(`- [Droit pénal général](${HOST}/expertise/droit-penal-general)`)
  ).toBe(true);
  expect(
    markdown.includes(`- [Cyber-criminalité](${HOST}/expertise/cyber-criminalite)`)
  ).toBe(true);
});
...
test('a document with no lead publishes no blockquote', () => {
  ...
  expect(field.split('\n').some(line => line.startsWith('> ')), field).toBe(false);

  const contact = contactMarkdown({ title: 'Contact' }, ctx());
  expect(contact.split('\n').some(line => line.startsWith('> ')), contact).toBe(false);
});
