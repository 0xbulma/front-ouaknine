import { leadSentence, llmsTxt, type LlmsTxtInput } from './llms-txt';
...
test('the guidance names the firm from the contact store, never a retyped literal', () => {
  ...
  expect(
    file.includes('Joignable au +33184162035, à cabinet@ouaknine-avocats.com.'),
    file
  ).toBe(true);
...
test('leadSentence reads through abbreviations, initials and decimals', () => {
  ...
  expect(leadSentence('Voir cf. Les autres cas. Puis fin.')).toBe(
  ...
  expect(leadSentence('Le cabinet plaide. « Une citation » suit.')).toBe(
    'Le cabinet plaide.'
  );
  // `!` and `?` end a sentence without any of the period rules applying.
  expect(leadSentence('Vous êtes convoqué ? Le cabinet intervient.')).toBe(
    'Vous êtes convoqué ?'
  );
