...
const url = (locale: Locale, path: string) => `${HOST}${escapeXml(withLocale(locale, path))}`;
...
const entry = ({
  ...
  <url>
    ...
    ${alternates
			.map(
				([hreflang, altPath]) =>
					`<xhtml:link rel="alternate" hreflang="${hreflang}" href="${url(hreflang, altPath)}"/>`,
			)
			.join("")}
...
${sets
	.flatMap((set) => set.map(([locale, path]) => entry({ locale, path, alternates: set, lastmod })))
	.join("")}
</urlset>`;
